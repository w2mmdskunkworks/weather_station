...
if (RewriteLogFileHeaders == True) :
	...
	logfile = open('/home/pi/logfile.txt', 'a+')
	logfile.write('Date,Time,WindSpeed,WindDir,WindVoltage,Rain,ClubhouseTemp,OutsideTemp,OutsideHumidity,Visible,IR,UV,LightningMessage,LightningCount,AirQualityMetric,AirQuality')
	logfile.write('\n')
	logfile.close()
...
if (ReadLightSensor == True):
	...
	#Light sensor is on BUS 2 - changed to bus 0
...
while True:
	#Turn on LED while measuring to test LED
	GPIO.output(WhiteLED,True)	
...
#Read from BMP280
	...
	if WriteLogFile == True:	
		...
		if (config.AS3935_Present): # Lightning sensor
			logfile.write(',')
